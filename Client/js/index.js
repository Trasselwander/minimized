...
api.elm.addEventListener("top10", () => { //player* but this code is never used.
    ...
    for (var i = 0; i < top10.length; i++) {
        ...
        trs[i].classList.remove("last");
    }

    for (var i = 10 - top10.length; i > 0; i--) {
        trs[10 - i].style.display = "none"; // if less than 10 players;
        trs[10 - i].classList.remove("last");
    }

    trs[top10.length - 1].classList.add("last");
});

api.elm.addEventListener("player", () => { //player* but this code is never used.
    if (!screens.overview.visible)
        return;
    ...
    if (player.userStats == null) {
        document.getElementById("stats_holder").style.display = "none";
        return;
    }
    document.getElementById("stats_holder").style.display = null;
    ...
    else for (var i = 0; i < sp_arr.length; i++)
